package specter3d.engine.utlis
{
	...
	import specter3d.engine.core.geom.OrientationStyle;
	...
	{
		/** rawData */
		public static const RAW_DATA:Float32Array=new Float32Array(16);
		...
		private static var decomposes:Vector.<Vector3D>=new Vector.<Vector3D>();
		...
		{
			var v0:Float32Array=a.rawData;
			var v1:Float32Array=b.rawData;
			var i:int;
		...
		public static function getRotation(m:Matrix3D, out:Vector3D=null):Vector3D
		{
			...
			{
				decomposes[0] = new Vector3D;
				decomposes[1] = new Vector3D;
				decomposes[2] = new Vector3D;
			}
			m.decompose(OrientationStyle.EulerAngles, decomposes);
			_vector.copyFrom(decomposes[1]);
		...
		public static function setRotation(m:Matrix3D, x:Number, y:Number, z:Number):void
		{
			...
			m.decompose(OrientationStyle.EulerAngles, decomposes);
			decomposes[1].x=(x * _toRad);
			decomposes[1].y=(y * _toRad);
			decomposes[1].z=(z * _toRad);
			m.recompose(decomposes);
		}
