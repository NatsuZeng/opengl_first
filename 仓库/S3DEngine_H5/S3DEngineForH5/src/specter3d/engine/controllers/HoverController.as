package specter3d.engine.controllers
{
	...
	import specter3d.engine.math.MathConsts;
	...
	public class HoverController extends LookAtController
	{
		 ...
		 */
		public function HoverController(targetObject:Camera3D = null, lookAtObject:Object3D = null, panAngle:Number = 0, tiltAngle:Number = 90, distance:Number = 1000, minTiltAngle:Number = -90, maxTiltAngle:Number = 90, minPanAngle:Number = NaN, maxPanAngle:Number = NaN, steps:uint = 8, yFactor:Number = 2, wrapPanAngle:Boolean = false)
		{
		...
		public override function update(interpolate:Boolean = true):void
		{
			...
			_sVector3D.x = _pos.x + _distance*Math.sin(_currentPanAngle*MathConsts.DEGREES_TO_RADIANS)*Math.cos(_currentTiltAngle*MathConsts.DEGREES_TO_RADIANS);
			_sVector3D.z = _pos.z + _distance*Math.cos(_currentPanAngle*MathConsts.DEGREES_TO_RADIANS)*Math.cos(_currentTiltAngle*MathConsts.DEGREES_TO_RADIANS);
			_sVector3D.y = _pos.y + _distance*Math.sin(_currentTiltAngle*MathConsts.DEGREES_TO_RADIANS)*_yFactor;
//			trace(_pos.x, _pos.y, _pos.z, _sVector3D.x, _sVector3D.y, _sVector3D.z, _currentPanAngle, _currentTiltAngle);
			_targetObject.transform.setPosition(_sVector3D.x, _sVector3D.y, _sVector3D.z);
			super.update();
		}
	}
}
