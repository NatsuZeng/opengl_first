package specter3d.engine.loaders.parsers
{
	...
	import specter3d.engine.core.geom.Quaternion;
	import specter3d.engine.core.geom.Vector3D;
	import specter3d.engine.loaders.data.Skeleton;
	import specter3d.engine.loaders.data.SkeletonJoint;
...
//				_animationSet = new SkeletonAnimationSet(_maxJointCount);
				...
				_mesh = new Mesh3D(new Geometry3D(), null);
				_geometry = _mesh.geometry;
				
				for (var i:int = 0; i < _meshData.length; ++i)
					_geometry.addSubGeometry(translateGeom(_meshData[i].vertexData, _meshData[i].weightData, _meshData[i].indices,_meshData[i].maxJointCount));
		 ...
		 */
		private function translateGeom(vertexData:Vector.<VertexData>, weights:Vector.<JointData>, indices:Vector.<uint>,maxJointCount:int):SubGeometry3D
		{
			...
			var pos:Vector3D;
			var subGeom:SubGeometry3D;
			var uvs:Vector.<Number> = new Vector.<Number>(len*2);
			var vertices:Vector.<Number> = new Vector.<Number>(len*3);
			var jointIndices:Vector.<Number> = new Vector.<Number>(len*maxJointCount);
			var jointWeights:Vector.<Number> = new Vector.<Number>(len*maxJointCount);
			var l:int;
